...
def generate_pdf_packet(
    ...
    header_data = [[
        ...
        Paragraph(
            f"Asset Claim Packet — {institution} &nbsp;|&nbsp; "
    ...
    # Checklist table: #  |  Document Name  |  Status
    # Col widths sum to PAGE_W
    cl_data = [[
        Paragraph("#",        S["label"]),
    ...
    overall_color = {"CRITICAL": RED, "MINOR": ORANGE, "OK": GREEN}[overall]
    story.append(Paragraph(
    ...
    if not comparisons:
        ...
        # ── TABLE COLUMN WIDTHS ────────────────────────────────
        # Total = 17 cm
        # Doc1 name | Name 1 | Doc2 name | Name 2 | Score | Status
        #  3.5 cm  | 3.5 cm |  3.5 cm  | 3.5 cm | 1.5cm | 1.5 cm
        COL_W = [3.5*cm, 3.5*cm, 3.5*cm, 3.5*cm, 1.5*cm, 1.5*cm]

        def _wrap(txt, max_chars=22):
            """Truncate long strings so they don't overflow cells."""
        ...
        style_cmds = [
            ...
            # Grid
            ("GRID",          (0, 0), (-1, -1), 0.4, GREY3),
            # Padding — keep tight so nothing overlaps
            ("TOPPADDING",    (0, 0), (-1, -1), 6),
            ("BOTTOMPADDING", (0, 0), (-1, -1), 6),
            ("LEFTPADDING",   (0, 0), (-1, -1), 5),
            ("RIGHTPADDING",  (0, 0), (-1, -1), 5),
            ("VALIGN",        (0, 0), (-1, -1), "MIDDLE"),
            # Score + Status columns centred
            ("ALIGN",         (4, 0), (5, -1), "CENTER"),
        ]

        for row_i, c in enumerate(comparisons, 1):
            score = round(c["score"])          # ← rounded integer
            status = c["status"]
            ...
            ct_data.append([
                Paragraph(_wrap(c["doc1"]),  S["body_small"]),
                Paragraph(_wrap(c["name1"]), ParagraphStyle("nm", fontName="Helvetica",
                    fontSize=8, textColor=BLACK)),
                Paragraph(_wrap(c["doc2"]),  S["body_small"]),
                Paragraph(_wrap(c["name2"]), ParagraphStyle("nm2", fontName="Helvetica",
                    fontSize=8, textColor=BLACK)),
                score_para,
                status_para,
            ])
    ...
    story.append(HRFlowable(width="100%", thickness=1, color=GREY3))
    story.append(Spacer(1, 0.2 * cm))
    story.append(_section_header("SECTION 4 — Next Steps Guide", S))
    story.append(Spacer(1, 0.25 * cm))

    next_steps = [
        ("1", "Resolve ALL name mismatches first.",
         "Visit a Notary Public for an affidavit (cost: Rs. 200–500 per affidavit). "
         "This is the single most common reason claims are rejected."),
        ("2", "Collect any MISSING documents listed in Section 1",
         "Do not visit the bank until every required document is in hand."),
        ("3", f"Submit at the {institution} branch.",
         "Ask specifically for the 'Deceased Account Claim Form'. "
         "Submit all documents + this covering letter + affidavits in one envelope."),
        ("4", "Get written acknowledgement.",
         "Ask the bank officer to stamp and sign your submission receipt. Keep it safely."),
        ("5", "Escalate if unsettled after 30 days.",
         f"Quote the circular: {rules.get('circular', 'applicable RBI/IRDAI circular')} "
         "in writing to the branch manager."),
        ("6", "RBI Banking Ombudsman (final escalation).",
         "File at: https://rbi.org.in/Scripts/Complaints.aspx"),
    ]

    step_data = []
    for num, bold, detail in next_steps:
        step_data.append([
    ...
    step_table = Table(step_data, colWidths=[0.8*cm, 16.2*cm])
    ...
    story.append(Spacer(1, 0.15 * cm))
    story.append(Paragraph(
        "Generated by DeathLedger  ·  Team Dispossible  ·  NMIT Bengaluru  ·  Vibe-a-thon 2026",
        S["footer"],
    ))
